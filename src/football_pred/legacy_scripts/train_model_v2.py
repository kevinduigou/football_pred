...
grid_search = GridSearchCV(
    ...
    scoring="neg_log_loss", n_jobs=-1, verbose=1, refit=True,
)
...
# --------------------------------------------------
# 6. Retrain on ALL data for production
# --------------------------------------------------
...
colors = ["#1565C0" if imp > 0.08 else "#42A5F5" if imp > 0.06 else "#90CAF9" for imp in feat_imp["importance"]]
ax.barh(feat_imp["feature"], feat_imp["importance"], color=colors)
ax.set_xlabel("Feature Importance (Gain)")
ax.set_title("XGBoost Feature Importance — Extended Dataset (2017-2025)")
plt.tight_layout()
...
ax.set_title("Confusion Matrix — Test Set (Chronological)")
plt.tight_layout()
...
ax.plot(results["validation_1"]["mlogloss"], label="Validation", alpha=0.8)
ax.set_xlabel("Boosting Round")
ax.set_ylabel("Multi-class Log Loss")
ax.set_title("XGBoost Training Curves — Extended Dataset")
ax.legend()
plt.tight_layout()
...
for i, (col, label) in enumerate(zip(["P_Home", "P_Draw", "P_Away"], ["Home", "Draw", "Away"])):
    ...
    cal = pred_df.groupby(bins, observed=False).apply(
        lambda g: pd.Series({
    ...
    axes[i].scatter(cal["mean_predicted"], cal["mean_actual"], s=cal["count"]*2, alpha=0.7, color="#1565C0")
...
next_match = pd.DataFrame([{
    "home_elo": 1620, "away_elo": 1580, "elo_diff": 40,
    "home_form_5": 2.0, "away_form_5": 1.2,
    "home_goals_for_avg": 1.8, "away_goals_for_avg": 1.3,
    "home_goals_against_avg": 0.9, "away_goals_against_avg": 1.1,
    "home_rest_days": 6, "away_rest_days": 4,
    "home_h2h_win_rate": 0.5, "away_h2h_win_rate": 0.3,
    "home_gd_form": 0.8, "away_gd_form": -0.2,
}])
...
# --------------------------------------------------
print("\n" + "="*65)
print("MODEL SUMMARY (Extended Dataset v2)")
print("="*65)
print(f"Data:              {len(df)} matches, 8 seasons (2017-2025)")
print(f"Leagues:           Premier League, La Liga, Serie A, Bundesliga, Ligue 1")
...
print(f"Baseline (Home):   {(y_test==0).mean():.4f}")
print("="*65)
