"""
Prédiction XGBoost v4 — Lorient (D) vs Lens (E)
"""
import pandas as pd
import numpy as np
import pickle
...
HOME_TEAM    = "Lorient"
AWAY_TEAM    = "Lens"
MATCH_DATE   = "2026-03-08 15:00:00"
OUT_CHART    = "/home/ubuntu/lorient_lens_prediction_v4.png"
OUT_REPORT   = "/home/ubuntu/lorient_lens_prediction_v4.md"
...
# Graphique 2 : radar des stats avancées
ax2 = axes[1]
categories = ['Tirs\ncadrés', 'Tirs\ntotaux', 'Tirs\nzone', 'Possession\n(%)',
              'Passes\n(÷10)', 'Précision\npasses (%)', 'Corners', 'xG']
...
width = 0.35
bars_h = ax2.bar(x_pos - width/2, h_vals, width, label=HOME_TEAM, color='#2196F3', alpha=0.85, edgecolor='white')
bars_a = ax2.bar(x_pos + width/2, a_vals, width, label=AWAY_TEAM,  color='#F44336', alpha=0.85, edgecolor='white')
...
Sur les 6 derniers matchs, Lorient affiche une forme contrastée : victoire contre Nantes (2-1) et Angers (2-0), mais deux nuls consécutifs contre Nice (3-3) et Auxerre (2-2), et une défaite à Brestois (0-2). La forme sur 5 matchs est de {feat['home_form_5']:.1f} pts/match.

### Forme récente de {AWAY_TEAM}
Lens traverse une période plus irrégulière : large victoire à Paris FC (5-0), mais défaite à Monaco (2-3), nul à Strasbourg (1-1), et défaite à Marseille (1-3). La forme sur 5 matchs est de {feat['away_form_5']:.1f} pts/match.

### Facteurs clés du modèle
Le modèle v4 intègre à la fois les indicateurs classiques (ELO, forme, H2H) et les statistiques avancées (xG, tirs, possession). L'ELO de Lens ({feat['away_elo']:.0f}) est supérieur à celui de Lorient ({feat['home_elo']:.0f}), mais l'avantage du terrain et les statistiques offensives récentes de Lorient compensent partiellement cet écart.

![Prédiction Lorient vs Lens](lorient_lens_prediction_v4.png)
"""
