"""
Updated pipeline: merge 34 new matches, rebuild advanced stats CSV,
rolling averages, retrain v2/v3/v4, compare.
Handles both old cache format (team_id keys) and new format (home_stats/away_stats).
"""
...
# ══════════════════════════════════════════════════════════════════════════
# STEP 1 – Build advanced_stats.csv from cache (dual format)
# ══════════════════════════════════════════════════════════════════════════
print("=" * 70)
print("STEP 1: Build advanced_stats.csv from cache (dual format)")
print("=" * 70)
...
for _, row in df.iterrows():
    ...
    if entry:
        # Detect format: new format has 'home_stats'/'away_stats' keys
        if "home_stats" in entry and "away_stats" in entry:
            # New format
            home_s = entry["home_stats"]
            away_s = entry["away_stats"]
            matched_count += 1
        elif "home_team" in entry and "away_team" in entry and "home_stats" not in entry:
            # Possibly new format with no stats
            pass
        else:
            # Old format: keys are team IDs, values have 'team_name' + stats
            keys = [k for k in entry.keys() if k not in ("home_team", "away_team", "home_stats", "away_stats")]
            if len(keys) >= 2:
                t1_key, t2_key = keys[0], keys[1]
                t1, t2 = entry[t1_key], entry[t2_key]
                n1 = t1.get("team_name", "").lower()
                n2 = t2.get("team_name", "").lower()
                dh = row["home_team"].lower()

                # Match home/away by team name
                if n1 in dh or dh in n1 or any(w in n1 for w in dh.split()):
                    home_s, away_s = t1, t2
                elif n2 in dh or dh in n2 or any(w in n2 for w in dh.split()):
                    home_s, away_s = t2, t1
                else:
                    home_s, away_s = t1, t2  # default: API returns home first
                matched_count += 1

    for sn in RAW_STATS:
        cn = clean(sn)
        rd[f"home_{cn}"] = home_s.get(sn)
        rd[f"away_{cn}"] = away_s.get(sn)

    if rd.get("home_expected_goals") is not None:
        xg_count += 1
...
print(f"Matched stats: {matched_count}/{len(df)}")
print(f"With xG: {xg_count}")

# ══════════════════════════════════════════════════════════════════════════
# STEP 2 – Build rolling averages
# ══════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 70)
...
for model_name, feature_cols in model_configs:
    ...
    for fn in fi["feature"]:
        if fn in ADVANCED_ROLLING:
            colors_fi.append("#FF6F00")
        elif fn in ["home_played_europe", "away_played_europe"]:
            colors_fi.append("#E53935")
        else:
            colors_fi.append("#1565C0")
    ...
    # Save v4 production model (trained on ALL data)
    if model_name == "advanced_v4":
        ...
        print(f"  Production model saved (trained on ALL {len(X)} matches).")
...
bars = axes[1].bar(models, ll_vals, color=colors_m)
...
for i, (m, k, c) in enumerate(zip(models, results_summary.keys(), colors_m)):
